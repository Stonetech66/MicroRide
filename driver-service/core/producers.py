...
async def publish_driver_created(driver_data:dict):
    ...
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.created')
...
async def publish_driver_status_updated(driver_data:dict):
    channel= await get_channel()
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.status.updated')
...
async def publish_ride_confirmed_driver(driver_data:dict):
    channel= await get_channel()
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.ride.confirmed')

# Handles publishing of event when a driver rejects a ride request
async def publish_ride_rejected(driver_data:dict):
    channel= await get_channel()
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.ride.rejected')

# Handles publishing of event to find ETA of a confirmed ride request
async def publish_find_ride_eta(driver_data:dict):
    channel= await get_channel()
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.find_eta')
...
async def publish_driver_new_ride_request(channel,driver_data:dict):
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.ride.new_request')
...
async def publish_find_new_driver(channel,driver_data:dict):
    driver_events= await channel.declare_exchange('driver-events', ExchangeType.TOPIC,)
    await driver_events.publish(Message(json.dumps(driver_data).encode()), routing_key='driver.ride.find_new_driver')
